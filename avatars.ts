// Send-only bot avatars (agent-bus P3) — the PURE half: parse the avatars config into a
// name→token map and resolve an endpoint to its avatar. No fs/path/grammy here (AVATARS_FILE + the
// disk read live in the daemon), so it's unit-testable in isolation like agent-bus.ts. An "avatar" is an
