...
- `renderMarkdown`: boolean (default `true`) — set `false` to send Claude's replies as plain text
...
- **Always** Read the file before Write — the daemon may have added pending
...
- Sender IDs are opaque strings (Telegram numeric user IDs). Don't validate
  format.
- The daemon re-reads `access.json` on every inbound message, so changes take
  ...
  is required for access changes.
