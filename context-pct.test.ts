...
// The stream cards' token line reads the same usage blocks and had the same defect on a fourth surface.
// `context` is a prompt SIZE and takes the last iteration; `output` is a turn TOTAL and keeps summing the
// top-level field, because every iteration's output really was generated by this turn.
test('the turn token counts read context per-iteration and output per-turn', () => {
  const f = write('turn.jsonl', [
    { type: 'user', message: { role: 'user', content: 'go' } },
    assistant({ output_tokens: 100, input_tokens: 0, cache_read_input_tokens: 50000, cache_creation_input_tokens: 0 }),
    assistant(TWO_ITERATIONS),
  ])
