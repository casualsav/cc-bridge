// A session card's foot carries the SESSION's numbers — and the 5h window is not one of them.
//
...
// Three cards, one per shape the removal touches: the redundancy case (everything), the card whose
// foot the 5h reading was HOLDING OPEN, and one that had nothing in the foot to begin with.
const SESSIONS = [
...
ok("FIXTURE: three cards rendered", cards.length === 3, cards.map(c => c.name).join(", "));
...
for (const c of cards) console.log(` ${String(c.name).padEnd(20)} h=${String(c.card.h).padEnd(7)} foot=${c.footItems ? c.footItems.join(" · ") : "—"}  tailAir=${c.tailAir} (padding ${c.padBottom})`);
console.log();
let bad = 0;
