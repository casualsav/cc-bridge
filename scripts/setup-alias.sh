...
MODE="${1:-off-mcp}"
COMMENT="# claude-tg: Telegram-bridged Claude Code launchers (${MODE})"
...
EOF
  ...
  mcp)
    read -r -d '' DEFS <<'EOF' || true
alias claude-tg='claude --dangerously-load-development-channels plugin:telegram@claude-tg --dangerously-skip-permissions'
EOF
    ;;
...
if [ -f "$RC" ]; then
  tmp=$(mktemp)
  grep -vE '^# [a-z][a-z0-9-]*: (launch|Telegram-bridged)|^claude-tg\(\)|^claude-yolo\(\)|^alias claude-tg=|^alias claude-yolo=' "$RC" > "$tmp" || true
  cat "$tmp" > "$RC"
  rm -f "$tmp"
fi
