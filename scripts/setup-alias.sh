...
#                ccb [--pin slack|discord] [slot] [account] -> tmux set -p @tg_bridge <slot> ; [tmux set -p @<pin> 1] ; [CLAUDE_CONFIG_DIR=~/.claude-<account>] claude --allow-dangerously-skip-permissions
#              `ccb` is the primary launcher (`claude-tg` is a kept back-compat alias). The
...
#              `ccb` = slot 1 (the default bridge); `ccb 2` routes to a
...
EOF
  ...
  mcp)
    read -r -d '' DEFS <<'EOF' || true
alias ccb='claude --dangerously-load-development-channels plugin:telegram@cc-bridge --dangerously-skip-permissions'
...
# Drop any prior block we wrote (our comment + the ccb/claude-tg/claude-yolo defs, alias or function
# form) so re-runs / mode switches replace cleanly, then append the fresh block.
if [ -f "$RC" ]; then
  tmp=$(mktemp)
  grep -vE '^# [a-z][a-z0-9-]*: (launch|Telegram-bridged)|^ccb\(\)|^claude-tg\(\)|^claude-yolo\(\)|^alias ccb=|^alias claude-tg=|^alias claude-yolo=' "$RC" > "$tmp" || true
  cat "$tmp" > "$RC"
  rm -f "$tmp"
fi
...
if [ "$MODE" = off-mcp ]; then
  echo "  Launch:  ccb              (default bridge, slot 1; claude-tg still works)"
  echo "           ccb 2            (second bridge — configure it first: /telegram:configure 2 <token>)"
else
  echo "  Launch:  ccb             (claude-tg still works)"
fi
