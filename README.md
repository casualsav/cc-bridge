...
Use `/harness` in a Claude Code topic to keep Claude Code's TUI, tools, skills, permissions, and transcript while changing only inference. Examples: `/harness codex`, `/harness codex gpt-5.6-terra`, `/harness kimi`, `/harness grok`, `/harness cursor`, and `/harness native` to return to Anthropic. The conversation is resumed in place and the choice persists with the topic. This is distinct from `/agent codex`, which runs the standalone Codex CLI. Set what NEW sessions of each role spawn on from `/settings` → 👤 Accounts → **💬 Chat** / **🧑‍💻 Coding** — the same provider vocabulary (Native Anthropic, every configured gateway, Codex, kimi, grok, cursor) as a default: the chat lane and fresh coding sessions (mini-app +, new topics, `tg spawn`, General) boot on the role's provider, while an existing session keeps whatever `/harness` choice it already has.

For any other Anthropic Messages-compatible provider, add a gateway straight from Telegram — no file editing, no restart. Open `/settings` → 👤 Accounts → **➕ Provider** and pick a preset: **MiniMax**, **DeepSeek**, and **Z.ai (GLM)** are built-in (base URL + a current model pre-filled — you only enter the API key), or choose **Custom** and reply with `name baseUrl model` for any other endpoint (auth defaults to `x-api-key`; append `bearer` or `none`). The API key is written to `.env` and your key message is deleted from the chat; a one-token Anthropic Messages preflight verifies it. The gateway is then usable live with `/harness gateway <name> [model]` **and** becomes a reorderable hop in your failover chain — when a Claude subscription hits its usage limit, the session falls over to it (same transcript, resumed in place, inference served by the gateway). The same Accounts panel shows every account, provider, and Codex in one ordered list — reorder with the ↑/↓ arrows, change a provider's model with ✏️, re-key with 🔑, remove with the 🗑 next to it.
