...
// `iterations` is the per-inference breakdown of ONE request; the top-level fields are its TOTAL.
...
export function currentTurnTokens(file: string): { output: number; context: number } {
  ...
  for (let i = start + 1; i < entries.length; i++) {
    ...
    // The LAST iteration, not the request total — a multi-iteration request sums the same context once
    // per iteration and reads ~N× the real fill (Telegram's transcript.ts carries the full account).
    // `output` keeps the total: every iteration's output really was generated by this turn.
