...
// Touched every 30s while the daemon's event loop is live — so ensure-slack-daemon can distinguish a
// wedged-but-alive daemon (stale mtime, but pid/socket still up) from a healthy one.
export const SLACK_HEARTBEAT_FILE = join(SLACK_STATE_DIR, 'daemon-heartbeat')
