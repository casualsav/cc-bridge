...
export class DiscordAdapter implements ChannelAdapter {
  ...
  private botId = ''
  private dmCache = new Map<string, string>()   // user snowflake → its opened DM channel id
  ...
  // Discord-concrete (NOT on ChannelAdapter): resolve a user snowflake to its DM channel id. A user
  // snowflake and a channel snowflake are indistinguishable by shape, so this is only called on the
  // sole-allowFrom fallback (where the id is known to be a user). Cached: a user's DM channel is stable.
  async openDm(userId: string): Promise<string> {
    const hit = this.dmCache.get(userId)
    if (hit) return hit
    const user = await this.c().users.fetch(userId)
