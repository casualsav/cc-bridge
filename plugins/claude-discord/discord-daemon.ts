...
import {
  ...
  DISCORD_INBOX_DIR, DISCORD_HEARTBEAT_FILE, loadDiscordEnv,
} from './discord-paths.ts'

const BRIDGE_PANE_OPT = '@tg_bridge'   // same adopt marker as Telegram — a Claude session is a Claude session
const PIN_PANE_OPT = '@discord_bridge'       // channel-specific pin: when set on any pane, it wins over auto-discovery
...
// ---- ctl IPC (the `dsc` CLI over the daemon socket; mirrors tgctl ↔ daemon.ts handleCall) ----
...
// allowlisted user. Explicit ids pass through verbatim.
async function resolveCtlTarget(chatArg: string | undefined): Promise<{ chatId: string; threadId?: string }> {
  if (chatArg && chatArg !== '.') return { chatId: chatArg }
  if (replyTarget) return { chatId: replyTarget.chatId, ...(replyTarget.threadId ? { threadId: replyTarget.threadId } : {}) }
  const allow = loadAllow()
  // The sole-allowFrom entry is a USER snowflake, not a channel — indistinguishable by shape on
  // Discord, so translate it to the user's DM channel id here (the only place we know it's a user).
  if (allow.length === 1) return { chatId: await channel.openDm(allow[0]) }
  throw new Error('no active chat — message the bot first, or pass an explicit channel id')
}
...
// Install the `dsc` bin shim so a bridged session can take Discord actions with no manual setup.
function provisionTooling(): void {
  try {
    const entry = join(import.meta.dir, 'dsc-ctl.ts')
    if (!existsSync(entry)) return
    const binDir = [join(homedir(), '.bun', 'bin'), join(homedir(), '.local', 'bin')].find(d => existsSync(d))
    if (binDir) {
      writeFileSync(join(binDir, 'dsc'), `#!/bin/sh\nexec bun ${entry} "$@"\n`, { mode: 0o755 })
      log(`provisioned dsc CLI → ${binDir}`)
    }
...
process.umask(0o077)
// The socket both holds the single-instance lock (socketAlive probes it) AND serves the `dsc` ctl
...
provisionTooling()
function touchHeartbeat(): void { try { writeFileSync(DISCORD_HEARTBEAT_FILE, String(Date.now()), { mode: 0o600 }) } catch {} }
touchHeartbeat()
...
function shutdown(): void {
  ...
  try { unlinkSync(DISCORD_SOCKET_PATH) } catch {}
  try { unlinkSync(DISCORD_HEARTBEAT_FILE) } catch {}
