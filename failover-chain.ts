...
//
// A "hop" is one Claude account or the (singular, today) Codex engine. Chain MEMBERSHIP is always
// every registered account plus a Codex hop when Codex is set up — login/snapshot/cap state is an
...
export function hopKey(h: FailoverHop): string {
  return h.kind === 'codex' ? 'codex' : `claude:${h.account}`
}

// Reconcile a saved order against current reality: keep the stored order, drop hops that no longer
// apply (a removed account, or Codex when it's not set up), then append anything new at the end —
// so a saved partial order still resolves to a complete chain covering every current hop.
export function resolveChain(stored: FailoverHop[], accountNames: string[], codexAvailable: boolean): FailoverHop[] {
