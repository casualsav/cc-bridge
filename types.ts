...
// Claude Code's five built-in output styles, spelled exactly as the CLI stores them in
// settings.json (read off the 2.1.239 binary — the strings are case-sensitive).
...
export type Access = {
  ...
  outputStyle?: OutputStyle  // Claude Code's own `outputStyle` settings key, carried into every daemon-originated launch as `--settings '{"outputStyle":"…"}'`. UNSET IS NOT A FALLBACK, it is "nothing configured": no flag at all, so an upgrade launches byte-identically. An explicitly named style — 'Default' INCLUDED — is passed, because --settings outranks the account's own settings.json and "default means no flag" is exactly what launched a mini-app spawn into the account's bypassPermissions (see spawnMode). Read ONCE at session start: it is part of the system prompt, so a running session picks a change up only on /clear or a restart
