...
export type Access = {
  ...
  spawnEffort?: string     // default effort for CODING sessions when the spawner passes no --effort; unset = spawnSession's own default. 'auto' = no fixed default: the caller's --effort IS the decision, and a spawn that names none falls to AUTO_EFFORT_FALLBACK and says so on the confirmation (never a launch value — `--effort auto` is refused)
